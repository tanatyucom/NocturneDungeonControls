...
したがって、右スティック回転についてはMOD方式を採用せず、reWASDとゲーム標準設定の組み合わせを推奨結果とする。ダッシュはreWASDから文字キー`P`を受け取り、`fldPlayerCalc()`実行中だけ通常移動のネイティブ基準速度`29`／`20`とワールドマップ専用定数`16`を1.6倍にする方式で実装した。Steam版1.0.4の病院内とワールドマップで実機確認済み。真VのZL相当へ寄せる場合はXboxのLTを`P`へ割り当て、ネイティブLTをミュートする。
